...
import vg.civcraft.mc.namelayer.mc.gui.folder.FolderElement;
import vg.civcraft.mc.namelayer.mc.gui.folder.GroupEntry;
import vg.civcraft.mc.namelayer.mc.gui.folder.GroupFolder;
...
public class GUIGroupOverview {
	...
	private GroupFolder constructFolderHierarchy() {
		...
		if (mapping.get("folders") != null) {
			...
			for (Map.Entry<String, JsonElement> entry : folderObj.entrySet()) {
		...
		Set<Group> allGroups = NameLayerPlugin.getInstance().getGroupTracker().getGroupsForPlayer(player.getUniqueId());
		if (mapping.get("groups") != null) {
			...
			for (Map.Entry<String, JsonElement> entry : folderObj.entrySet()) {
				...
				existing.add(entry.getKey());
				allGroups.remove(GroupAPI.getGroup(entry.getKey()));
			}
		}
			...
			public void clicked(final Player p) {
				...
				new Dialog(p, NameLayerPlugin.getInstance()) {
					...
					public void onReply(String[] message) {
						...
						ArtemisPlugin.getInstance().getRabbitHandler().sendMessage(new RabbitCreateGroup(p.getUniqueId(), groupName));
						this.end();
					}
				};
			...
			new Dialog(p, NameLayerPlugin.getInstance()) {
				...
				public void onReply(String[] message) {
					...
					new Dialog(p, NameLayerPlugin.getInstance()) {
						...
						public void onReply(String[] message) {
							this.end();
							ArtemisPlugin.getInstance().getRabbitHandler().sendMessage(
									new RabbitJoinGroup(player.getUniqueId(), group.getName(),
											String.join(" ", message)));
						}
					};
				}
	...
	public static ItemStack getHashedItem(int seed) {
		Map<Integer, ItemStack> idToItem = new HashMap<>();
		int counter = 0;
		...
		int number = seed % idToItem.size();
		if (number < 0) {
			number = 0;
		}
		return idToItem.get(number).clone();
	}
